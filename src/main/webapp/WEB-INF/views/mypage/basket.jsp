	    ...
	    <div class="mypageContainer">
		        ...
		        <div class="profileBox">
		            ...
		            <div class="profileInfo">
		                <div class="productInfo">
		                    <span><strong>찜한상품</strong></span>
		                </div>
		                <form id="countpost" action="mypage/toMyStore?curPage=1">
		                <div class="productCount">
		                    <span><strong>${bcnt}</strong></span>
	        ...
	        <div class="content">
	            <div class="categoryText">
	                <h3><strong>찜 목록</strong></h3>
	            </div>
	            <div class="totalCount">
	                <span><strong>전체 ${bcnt}개</strong></span>
	            </div>
	            <div class="mypost">
			...
			<ul >
				<c:if test="${naviMap.needPrev eq true }">
					<a href="/mypage/toBasket?curPage=${naviMap.startNavi-1}&post_addr=${naviMap.post_addr}&user_id=${naviMap.user_id}">
					</a>
				</c:if>
				<c:forEach var="pageNum" begin="${naviMap.startNavi}" end="${naviMap.endNavi}" step="1">
					<c:if test="${naviMap.curPage eq pageNum}">
						<a class="is-active" href="/mypage/toBasket?curPage=${pageNum}&post_addr=${naviMap.post_addr}&user_id=${naviMap.user_id}">
							${pageNum}
						</a>
					</c:if>
					<c:if test="${naviMap.curPage ne pageNum}">
						<a class="pBtn" href="/mypage/toBasket?curPage=${pageNum}&post_addr=${naviMap.post_addr}&user_id=${naviMap.user_id}">
							${pageNum}
						</a>
					</c:if>	
				</c:forEach>
				<c:if test="${naviMap.needNext eq true }">
					<a href="/mypage/toBasket?curPage=${naviMap.endNavi+1}&post_addr=${naviMap.post_addr}&user_id=${naviMap.user_id}">
					</a>
				</c:if>
			</ul>
		</nav>
